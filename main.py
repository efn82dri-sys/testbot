...
_vip_banner_cache: dict[str, str] = {}  # cat_id -> file_id
...
async def handle_user_menu(callback: CallbackQuery, state: FSMContext):
    ...
    if key == "vip":
        if VIP_GROUP_CHAT_ID is None:
            ...
            return
        # ارسال پیام VIP با بنر تصویری
        await render_and_send_vip_page(callback.message, 0)
        await callback.answer()
        return
...
async def render_and_send_vip_page(message: Message, index: int, edit: bool = False):
    ...
    # ساخت متن (کپشن) - شامل شمارنده در خط جداگانه
    ...
    # ارسال یا ویرایش پیام با بنر
    image_file_id = cat.get("image_file_id")
    
    if edit:
        # ویرایش پیام موجود - اگر بنر داره، باید پیام رو حذف و دوباره ارسال کنیم
        # چون ویرایش نمیتونه نوع پیام (متن به عکس) رو تغییر بده
        try:
...
async def cb_vip_cancel_payment(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await callback.message.answer("عملیاتِ خرید لغو شد.", reply_markup=user_panel_keyboard())
    await callback.answer()
